...
package edu.boun.edgecloudsim.applications.Project.Priorites.sample_app4;
	...
	public int getDeviceToOffload(Task task) {
		...
		if(simScenario.equals("SINGLE_TIER")){
			...
			if(policy.equals("FUZZY_BASED")){
				...
				//appPri
				// 👇 adjust IDs to your sim (e.g., AR=0, HEALTH=1, HEAVY=2)
				final int APP_AR = 0, APP_HEALTH = 1, APP_HEAVY = 2;

				double appPriority = 0.40; // default for heavy/others
				if (task.getTaskType() == APP_HEALTH) {
					appPriority = 1.00;    // critical
				} else if (task.getTaskType() == APP_AR) {
					appPriority = 0.75;    // elevated
				}


				fis2.setVariable("app_priority", appPriority);
				...
				*/
				...
				//appPi
				fis1.setVariable("app_priority", appPriority);
		        // Evaluate
		        fis1.evaluate();
